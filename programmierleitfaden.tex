...
\author{Anne-Sophie St\"o{}hlker, Arno R\"oder, Axel Albrecht,\\
Bernhard B\"o{}sch, Christian Vonderach,\\
Dominik Cullmann, Elke Erhardt, Franziska Berg, Gerald K\"a{}ndler,\\ 
Marco Reimann}
\uppertitleback{%
    ...
    Auf \url{https://github.com/fvafrCU/programmierleitfaden} findest Du alle
    Quellen und Beispiele sowie die aktuelle 
    \href{https://raw.githubusercontent.com/fvafrCU/programmierleitfaden/master/programmierleitfaden.ps}
    {Version dieses Leitfadens} in
    \href{http://de.wikipedia.org/wiki/PostScript}{PostScript}.
...
Schreibe Code den Du in sechs Monaten noch verstehen wirst.
...
\lstinputlisting[float=h!, label=lst:r_uncool, language=R, firstline = 2]%
{uncool.r}
\lstinputlisting[float=h!, label=lst:r_cool, language=R, firstline = 2]{cool.r}
...
\minisec{Welches Englisch?} Vielen mag Britisches Englisch \"a{}sthetischer
...
Namen und
zur Trennung von W\"o{}rtern in Namen:
\begin{itemize}
\item UpperCamelCase
\item lowerCamelCase
\item lowercase
...
Die gute Benennung von Bezeichnern (Variablen, Funktionen, Klassen, \ldots)
...
im Anhang \ref{app:R}).
\lstinputlisting[float=h, label=lst:write_readme, firstline=14
]{write_readme.r}
...
\lstinputlisting[float=h, label=lst:doccom2, firstline = 2, lastline=13]%
{write_readme.r}
...
umwandeln
kannst.
\lstinputlisting[float=h, label=lst:package, firstline = 2]%
...
\code{document}, wir erkl\"a{}ren sie hier nicht, da dies schon in der
...
. 
\lstinputlisting[float=h, label=lst:roxy_ex, linerange={3-4}]
...
einfach automatisiert gegen den dokumentierten Code testen l\"a{}sst (siehe
